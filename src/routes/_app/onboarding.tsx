...
import { ClipboardList, UserPlus, CheckCircle2, Plus, Loader2, Trash2 } from 'lucide-react'
import {
  getOnboarding,
  createOnboarding,
...
import { Card, KpiCard, Avatar, Badge } from '#/components/ui'
          ...
          {d.onboardings.length ? d.onboardings.map((o) => (
            <div key={o.id} className="rounded-lg border border-slate-200">
              <button onClick={() => setOpen(open === o.id ? null : o.id)} className="flex w-full items-center gap-3 px-4 py-3 text-left">
                <Avatar name={o.candidateName} size={38} />
                <div className="min-w-0 flex-1">
                ...
                <div className="hidden w-40 sm:block">
                  <div className="h-2 rounded-full bg-slate-100">
                    <div className="h-2 rounded-full bg-blue-600" style={{ width: `${o.progress}%` }} />
                  </div>
                </div>
                <Badge tone={o.status === 'completed' ? 'ok' : 'in_progress'} label={o.status === 'completed' ? 'Completed' : `${o.progress}%`} />
              </button>
              {open === o.id ? (
                <>

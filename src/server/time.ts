...
export const getTimeTracking = createServerFn({ method: 'GET' }).handler(async () => {
  ...
  let myWeekHours = 0
  if (empId) {
    sessions = (await sql`select id, clock_in, clock_out, hours_worked, status from time_entries
...
export const clockIn = createServerFn({ method: 'POST' }).handler(
  async (): Promise<Result<null>> => {
    ...
    const open = (await sql`select id from time_entries
      where employee_id = ${me.employeeId} and day = ${today} and clock_out is null and status = 'active' limit 1`)[0] as
      | { id: number }
      | undefined
...
export const clockOut = createServerFn({ method: 'POST' }).handler(
  async (): Promise<Result<null>> => {
    ...
    const today = todayIso()
    const entry = (await sql`select id from time_entries

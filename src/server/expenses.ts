...
export const getExpenses = createServerFn({ method: 'GET' }).handler(
  async () => {
    ...
    const myReimbursed = empId
      ? n(
          (
            await sql`select coalesce(sum(amount),0) s from expenses where employee_id=${empId} and status='reimbursed'`
          )[0].s,
        )
      : 0
    const myPending = empId
      ? n(
          (
            await sql`select coalesce(sum(amount),0) s from expenses where employee_id=${empId} and status in ('pending','approved')`
          )[0].s,
        )
      : 0
    ...
    const orgPending = n(
      (
        await sql`select coalesce(sum(amount),0) s from expenses where status='pending'`
      )[0].s,
    )
...
export const decideExpense = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }): Promise<Result<null>> => {
    try {
      ...
      const me = await getSessionUser()
      if (!canApprove(me))
        return { ok: false, error: 'Not authorised for this action' }

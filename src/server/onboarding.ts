...
export const getOnboarding = createServerFn({ method: 'GET' }).handler(
  async () => {
    const sql = requireDb()
    const rows =
      (await sql`select id, candidate_name, role, department, start_date, status, progress, employee_id
    from onboardings order by created_at desc`) as Array<any>
    const tasks =
      (await sql`select id, onboarding_id, task, category, done from onboarding_tasks order by sort_order`) as Array<any>
    const byOnb: Record<number, Array<any> | undefined> = {}
    for (const t of tasks) (byOnb[t.onboarding_id] ??= []).push(t)

    const noteRows =
      (await sql`select id, onboarding_id, note, done, created_at from onboarding_notes order by created_at`) as Array<any>
...
export const toggleOnboardingNote = createServerFn({ method: 'POST' })
  .validator((d: unknown) =>
    z.object({ noteId: z.number().int().positive() }).parse(d),
  )
  .handler(async ({ data }): Promise<Result<null>> => {
    const denied = await requireApprover()
    if (denied) return denied
...
export const createOnboarding = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }): Promise<Result<null>> => {
    ...
    const params = TASK_TEMPLATE.flatMap((t, i) => [
      onb.id,
      t.task,
      t.category,
      i,
    ...
    await sql.query(
      `insert into onboarding_tasks (${cols.join(',')}) values ${values}`,
      params,
    )
    return { ok: true, data: null }
  })
...
export const toggleOnboardingTask = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }): Promise<Result<{ progress: number }>> => {
    ...
    const sql = requireDb()
    const task = (
      await sql`select id, onboarding_id, done from onboarding_tasks where id=${data.taskId}`
    )[0] as { id: number; onboarding_id: number; done: boolean } | undefined
    ...
    if (progress === 100) {
      const onb = (
        await sql`select candidate_name, email, department, role, start_date, employee_id from onboardings where id=${onbId}`
      )[0] as
        | {

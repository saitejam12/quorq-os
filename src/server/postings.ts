...
export const getPostings = createServerFn({ method: 'GET' }).handler(async () => {
  const sql = requireDb()
  ...
  const publishedCount = n(
    (await sql`select count(*) c from job_openings where published and posting_status='active'`)[0].c,
  )
  const openedThisMonth = n(
    (await sql`select count(*) c from job_openings where posting_status='active' and opened_date >= date_trunc('month', CURRENT_DATE)`)[0].c,
  )
  ...
  .handler(async ({ data }): Promise<Result<null>> => {
    try {
      ...
      const template = (await sql`
        select id, title, category, description from jd_templates where id = ${data.templateId}`)[0] as
        | JdTemplate
        | undefined
...
export const deactivatePosting = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }): Promise<Result<null>> => {
    try {
      ...
      const me = await getSessionUser()
      if (!canApprove(me)) {
        return { ok: false, error: 'Only ops and master can deactivate postings' }
      }
...
export const createTemplate = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }): Promise<Result<null>> => {
    try {
      const sql = requireDb()
      const me = await getSessionUser()
      if (!canApprove(me)) return { ok: false, error: 'Only ops and master can manage templates' }
      await sql`
...
export const updateTemplate = createServerFn({ method: 'POST' })
  .validator((d: unknown) => TemplateFields.extend({ id: z.number().int().positive() }).parse(d))
  .handler(async ({ data }): Promise<Result<null>> => {
    try {
      const sql = requireDb()
      const me = await getSessionUser()

...
async function headcount() {
  ...
  const byStatus =
    (await sql`select status, count(*) c from employees group by status`) as Array<any>
  const get = (st: string) => n(byStatus.find((r) => r.status === st)?.c)
  const newThisMonth = n(
    (
      await sql`select count(*) c from employees where date_of_joining >= date_trunc('month', CURRENT_DATE)`
    )[0].c,
  )
  return {
    total,
...
async function todayAttendance() {
  ...
  const late = n(
    (
      await sql`select count(*) c from attendance_records where day=(select max(day) from attendance_records) and late`
    )[0].c,
  )
  return {
    total,
...
async function attendanceTrend() {
  ...
  return rows
    .map((r) => ({
      ...
      value: pct(n(r.present), n(r.total)),
    }))
    .reverse()
}
...
function attritionTrendShape(currentRate: number) {
  ...
  const shape = [
    0.62, 0.66, 0.7, 0.74, 0.78, 0.82, 0.86, 0.9, 0.93, 0.96, 0.98, 1.0,
  ]
  return labels.map((label, i) => ({
    label,
    value: round1(shape[i] * currentRate),
  }))
}
...
// EXECUTIVE OVERVIEW
// ---------------------------------------------------------------------------
export const getExecutive = createServerFn({ method: 'GET' }).handler(
  async () => {
    const sql = requireDb()
    ...
    const compliance =
      (await sql`select label, value, tone from compliance_items order by sort_order`) as Array<any>

    return {
      headcount: hc,
      attrition: {
        rate: att.rate,
        deltaVsQ3: C.attritionDeltaVsQ3,
      ...
      velocity: {
        ...
        target: C.timeToHireTarget,
        offerAccept: C.offerAcceptRate,
      },
    }
  },
)
...
export const getAttendance = createServerFn({ method: 'GET' }).handler(
  async () => {
    ...
    const leaveTaken = n(
      (
        await sql`select coalesce(sum(days),0) s from leave_requests where status='approved'`
      )[0].s,
    )
    const empCount = n(
      (await sql`select count(*) c from employees where status<>'exited'`)[0].c,
    )
    ...
    const overtime = n(
      (
        await sql`select coalesce(sum(overtime_hours),0) s from attendance_records where day >= date_trunc('month', CURRENT_DATE)`
      )[0].s,
    )
    const lateMonth = n(
      (
        await sql`select count(*) c from attendance_records where day >= date_trunc('month', CURRENT_DATE) and late`
      )[0].c,
    )
    const earlyMonth = n(
      (
        await sql`select count(*) c from attendance_records where day >= date_trunc('month', CURRENT_DATE) and early_exit`
      )[0].c,
    )
    ...
    const zeroLeave = n(
      ...
      and not exists (select 1 from leave_requests l where l.employee_id = e.id)`
      )[0].c,
    )
    ...
    return {
      ...
      absenteeism: {
        rate: C.absenteeismRate,
        delta: C.absenteeismDelta,
        target: C.absenteeismTarget,
      },
      overtime: {
...
// ATTRITION & RETENTION
// ---------------------------------------------------------------------------
export const getAttrition = createServerFn({ method: 'GET' }).handler(
  async () => {
    const sql = requireDb()
    const att = await attritionRate()
    ...
    const byDept =
      (await sql`select department, count(*) c from exits group by department order by c desc`) as Array<any>
    const reasons =
      (await sql`select reason, count(*) c from exits group by reason`) as Array<any>
    const reasonOrder = [
      'salary',
      'growth',
      'management',
    ...
    const tenure =
      (await sql`select tenure_bucket, count(*) c from exits group by tenure_bucket`) as Array<any>
    ...
    const tenureAtExit = ['under_1yr', '1_2yr', '2_4yr', '4yr_plus'].map(
      (t) => ({
    ...
    const regrettable = n(
      (await sql`select count(*) c from exits where regrettable`)[0].c,
    )
    const counterOffer = n(
      (await sql`select count(*) c from exits where counter_offer_accepted`)[0]
        .c,
    )
    ...
    return {
      attrition: {
        rate: att.rate,
        benchmark: C.industryAttritionAvg,
        exitsThisMonth: C.exitsThisMonth,
        voluntary: C.voluntaryThisMonth,
      },
      retention: {
      ...
      cost: {
        totalL: round1(att.exits * C.costPerExitL),
        perExitL: C.costPerExitL,
        exitsYTD: att.exits,
      },
      attritionByDept: byDept.map((r) => ({
        label: r.department,
        value: n(r.c),
      })),
      exitReasons,
      flightRiskEmployees: flightRisk.map((r) => ({
        name: r.name,
        dept: r.department,
        level: r.flight_risk,
      })),
...
export const getTalent = createServerFn({ method: 'GET' }).handler(async () => {
  ...
  const openCount = n(
    (
      await sql`select count(*) c from job_openings where posting_status='active'`
    )[0].c,
  )
  const critical = n(
    (
      await sql`select count(*) c from job_openings where is_critical and posting_status='active'`
    )[0].c,
  )
  const byCat =
    (await sql`select category, count(*) c from job_openings where posting_status='active' group by category`) as Array<any>
  ...
  const sources =
    (await sql`select source, count(*) c from applications group by source`) as Array<any>
  ...
  const female = n(
    (await sql`select count(*) c from applications where gender='female'`)[0].c,
  )
  ...
  return {
    ...
    timeToHire: {
      avg: C.avgTimeToHire,
      target: C.timeToHireTarget,
      best: C.bestTimeToHire,
    },
    offerAccept: {
      rate: C.offerAcceptRate,
      made: C.offersMade,
      accepted: C.offersAccepted,
      declined: C.offersDeclined,
    },
    costPerHire: {
      value: C.costPerHireK,
      agency: C.agencyCostK,
      referral: C.referralCostK,
    },
    funnel,
...
// ---------------------------------------------------------------------------
export const getWorkforce = createServerFn({ method: 'GET' }).handler(
  async () => {
    const sql = requireDb()
    const total = n((await sql`select count(*) c from employees`)[0].c)
    const byDept =
      (await sql`select department, count(*) c from employees group by department order by c desc`) as Array<any>
    const byType =
      (await sql`select employment_type, count(*) c from employees group by employment_type`) as Array<any>
    const byLoc =
      (await sql`select location, count(*) c from employees group by location order by c desc`) as Array<any>
    const byGender =
      (await sql`select gender, count(*) c from employees group by gender`) as Array<any>
    const topPerformers = n(
      (await sql`select count(*) c from employees where is_top_performer`)[0].c,
    )
    const avgRating = round1(
      n((await sql`select avg(performance_rating) a from employees`)[0].a),
    )
    ...
    const avgTenureDays = n(
      (
        await sql`select avg(CURRENT_DATE - date_of_joining) a from employees`
      )[0].a,
    )
    ...
    return {
      ...
      tenure: tenureOrder.map((t) => ({
        label: t,
        value: n(tenure.find((x) => x.bucket === t)?.c),
      })),
      performance: perfOrder.map((p) => ({
        label: p,
        value: n(perfBands.find((x) => x.band === p)?.c),
      })),
      topPerformers,
      avgRating,
...
export const getReports = createServerFn({ method: 'GET' }).handler(
  async () => {
    const sql = requireDb()
    const prebuilt =
      (await sql`select title, subtitle, category, formats, icon from prebuilt_reports order by id`) as Array<any>
    const scheduled =
      (await sql`select name, cadence, tone from scheduled_reports order by id`) as Array<any>
    const statutory =
